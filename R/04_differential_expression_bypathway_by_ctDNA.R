...
fit = eBayes(fit)
setof_allsignatures = topTable(fit, coef = "status", number = Inf) %>%
...
plot_ = setof_allsignatures %>%
	...
	dplyr::mutate(is_apoptosis = case_when(
		grepl("APOPTOSIS|APOPTOTIC|NECROSIS", signature_name, perl = TRUE, fixed = FALSE, ignore.case = TRUE) ~ "Yes",
		TRUE ~ "No"
	)) %>%
	dplyr::mutate(is_apoptosis = factor(is_apoptosis, levels = c("Yes", "No"), ordered = TRUE)) %>%
	dplyr::arrange(desc(is_apoptosis)) %>%
	ggplot(aes(x = logFC, y = (1/P.Value), fill = is_apoptosis, color = is_apoptosis, alpha = is_apoptosis)) +
	geom_point(stat = "identity", size = 2, shape = 21) +
	geom_hline(yintercept=1/.05, color = "goldenrod3", linetype = 2, size = .25) +
	geom_vline(xintercept=c(-2.5, 2.5), color = "goldenrod3", linetype = 2, size = .25) +
	scale_fill_manual(values = c("Yes" = "#fc9272",
				      "No" = "#ffffff")) +
	...
	guides(fill = guide_legend(title="Apoptosis\nRelated", override.aes = list(size = 3))) +
	geom_text_repel(data = subset(setof_allsignatures %>%
				      ...
				      dplyr::filter(is_apoptosis=="Yes"),
				      (logFC>2.5|logFC<(-2.5)) & (1/P.Value)>(1/.05)),
			mapping = aes(x = logFC, y = (1/P.Value), label = signature_name),
			size = 1.3, segment.size = 0.35, force = 1, show.legend = FALSE, inherit.aes = FALSE)

pdf(file = "../res/Differential_Expression_by_Pathway_ctDNA.pdf", width = 5, height = 4)
print(plot_)
dev.off()

setof_allsignatures %>%
dplyr::select(signature_name,
	      Log2_FC = logFC,
	      Mean_Expression = AveExpr,
	      P_value = P.Value,
	      FDR = adj.P.Val,
	      Number_of_Genes = n,
	      MSigDB_Collection = category) %>%
readr::write_tsv(path = "../res/Differential_Expression_by_Pathway_ctDNA.txt", col_names = TRUE, append = FALSE)
...
data_ = exprs(BroadGeneSet_SSGSEA) %>%
	...
	dplyr::filter(signature_name %in% (setof_allsignatures %>%
					   ...
					   dplyr::filter(grepl("APOPTOSIS|APOPTOTIC|NECROSIS|NECROPTIC|DEATH", signature_name, perl = TRUE, fixed = FALSE)) %>%
					   .[["signature_name"]])) %>%
	reshape2::melt() %>%
