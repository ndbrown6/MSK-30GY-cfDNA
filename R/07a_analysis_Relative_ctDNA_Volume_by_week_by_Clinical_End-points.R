...
plot_ = idx_metrics_ft %>%
	...
	geom_density_ridges2(stat = "density_ridges", bandwidth = .35, scale = 1.75, color = "black", alpha = 1) +
...
plot_ = idx_metrics_ft %>%
	...
	ggplot(aes(x = timepoint_weeks_since_start_of_RT, y = aligned_reads,
		   fill = timepoint_weeks_since_start_of_RT, color = timepoint_weeks_since_start_of_RT)) +
	geom_line(aes(group = patient_id_mskcc), size = .1, alpha = 1, color = "black") +
	geom_point(stat = "identity", shape = 21, size = 2, alpha = 1) +
...
pdf(file = "../res/Number_Read_Pairs_by_Wk_by_Patient.pdf", width = 3, height = 2.5*1.25)
print(plot_)
dev.off()
...
pdf(file = "../res/Mean_AF_by_Wk_by_Patient.pdf", width = 3, height = 2.5*1.25)
print(plot_)
dev.off()
...
plot_ = clinical %>%
	...
	dplyr::mutate(timepoint_weeks_since_start_of_RT = factor(x = timepoint_weeks_since_start_of_RT,
								 levels = c("wk5", "wk3", "wk2", "wk1", "Pre-treatment"),
								 ordered = TRUE)) %>%
	ggplot(aes(x = MRI_volume*0.001, y = timepoint_weeks_since_start_of_RT,
		   fill = timepoint_weeks_since_start_of_RT)) +
	...
	scale_y_discrete(breaks = c("wk5", "wk3", "wk2", "wk1", "Pre-treatment"),
			 labels = c("Week 5", "Week 3", "Week 2", "Week 1", "Pre-treatment")) +
	xlab(expression("MRI Volume ("~cm^3~")")) +
	ylab("") +
	theme_classic() +
	theme(axis.title.x = element_text(margin = margin(t = 20)),
	      axis.title.y = element_text(margin = margin(r = 20)),
...
pdf(file = "../res/MRI_Volume_by_Wk_by_Patient.pdf", width = 3, height = 2.5*1.25)
print(plot_)
dev.off()
...
plot_ = clinical %>%
	...
	dplyr::mutate(timepoint_weeks_since_start_of_RT = factor(x = timepoint_weeks_since_start_of_RT,
								 levels = c("wk5", "wk3", "wk2", "wk1", "Pre-treatment"),
								 ordered = TRUE)) %>%
	ggplot(aes(x = ADC, y = timepoint_weeks_since_start_of_RT,
		   fill = timepoint_weeks_since_start_of_RT, color = timepoint_weeks_since_start_of_RT)) +
	geom_density_ridges2(stat = "density_ridges", bandwidth = .025, scale = 1.5, color = "black", alpha = 1) +
	scale_fill_viridis(discrete = TRUE) +
	scale_x_log10(labels = scientific_10) +
	scale_y_discrete(breaks = c("wk5", "wk3", "wk2", "wk1", "Pre-treatment"),
			 labels = c("Week 5", "Week 3", "Week 2", "Week 1", "Pre-treatment")) +
...
pdf(file = "../res/ADC_by_Wk.pdf", width = 3*1.15, height = 2.5*1.25)
print(plot_)
dev.off()
...
pdf(file = "../res/ADC_by_Wk_by_Patient.pdf", width = 3, height = 2.5*1.25)
print(plot_)
dev.off()
