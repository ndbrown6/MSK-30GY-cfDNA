...
# HPV read counts upper versus lower median
...
plot_ = setof_allsignatures %>%
	...
	dplyr::group_by(is_pathway) %>%
	dplyr::summarize(n = n()) %>%
	dplyr::filter(is_pathway!="Other") %>%
	dplyr::mutate(f = 100*n/sum(n)) %>%
	ggplot(aes(x = is_pathway, y = f, fill = is_pathway)) +
...
pdf(file = "../res/Fraction_Pathway_median.pdf", width = 1.75, height = 4)
print(plot_)
dev.off()
...
pdf(file = "../res/Fraction_Pathway_outer_quartile.pdf", width = 1.75, height = 4)
print(plot_)
dev.off()
...
pdf(file = "../res/Fraction_Pathway_postive_negative.pdf", width = 1.75, height = 4)
print(plot_)
dev.off()
