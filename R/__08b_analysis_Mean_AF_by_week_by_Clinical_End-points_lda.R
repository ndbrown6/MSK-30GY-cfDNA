...
'Class_NoCv' <- function(data) {
	fit = lda(composite_end_point ~ ., data = data)
	prd = predict(object = fit, newdata = data)$class
...
'Class_10Cv' <- function(data) {
	...
	for (j  in 1:100) {
		...
		for (i in 1:length(folds)) {
			...
			tmp.fit = lda(composite_end_point ~ ., data = train)
			tmp.prd = predict(tmp.fit, newdata = test)$class
...
'ROC' <- function(data) {
	fit = lda(composite_end_point ~ ., data = data)
	prd = predict(object = fit, newdata = data)$posterior[,"1"]
...
errors_lda = list(Training_Error = Training_Error,
		  Test_Error = Test_Error,
		  ROC_Curve = ROC_Curve)

save(errors_lda, file = "../res/Classifier_Lda.RData")