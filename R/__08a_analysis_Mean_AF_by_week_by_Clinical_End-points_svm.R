...
'Class_NoCv' <- function(data) {
	fit = svm(composite_end_point ~ ., data = data, scale = FALSE)
...
'Class_10Cv' <- function(data) {
	...
	for (j  in 1:100) {
		...
		for (i in 1:length(folds)) {
			...
			tmp.fit = svm(composite_end_point ~ ., data = train, scale = FALSE)
...
'ROC' <- function(data) {
	fit = svm(composite_end_point ~ ., data = data, scale = FALSE, probability = TRUE)
	prd = attributes(predict(object = fit, newdata = data, probability = TRUE))$probabilities[,"1"]
...
errors_svm = list(Training_Error = Training_Error,
		  Test_Error = Test_Error,
		  ROC_Curve = ROC_Curve)

save(errors_svm, file = "../res/Classifier_Svm.RData")