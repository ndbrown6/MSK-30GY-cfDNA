...
plot_ = posterior_smry %>%
	...
	dplyr::select(patient_id_mskcc, timepoint_days_since_start_of_RT, mean_af, max_af, median_af, Is_ctDNA, frac_ctDNA) %>%
	reshape2::melt(id.vars = c("patient_id_mskcc", "timepoint_days_since_start_of_RT", "Is_ctDNA"), measure.vars = c("mean_af", "max_af", "median_af", "frac_ctDNA"),
