...
smry_ = test_ %>%
	...
	dplyr::mutate(`Pr(x=0)` = res_$posterior[,"-ve"]) %>%
	dplyr::mutate(`Pr(x=1)` = res_$posterior[,"+ve"]) %>%
	dplyr::as_tibble() %>%
	readr::type_convert() %>%
	dplyr::left_join(mutation_smry %>%
	     		 dplyr::filter(FILTER == "PASS") %>%
			 dplyr::group_by(Tumor_Sample_Barcode) %>%
			 dplyr::summarize(max_af = max(t_maf),
					  median_af = median(t_maf),
					  mean_af = mean(t_maf)) %>%
			 dplyr::ungroup() %>%
			 dplyr::rename(sample_name = Tumor_Sample_Barcode),
			 by = "sample_name") %>%
	dplyr::left_join(manifest, by = "sample_name") %>%
	dplyr::mutate(index = gsub("CTMS-", "", patient_id_mskcc, fixed = TRUE)) %>%
	readr::type_convert() %>%
	dplyr::arrange(index) %>%
