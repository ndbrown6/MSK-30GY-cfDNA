...
for (i in 1:length(target_names)) {
	fit_ = MASS::lda(Is_ctDNA ~ ., data = data_ %>% dplyr::select(c("Is_ctDNA", paste0(target_names[i], "_READ_COUNT"), paste0(target_names[i], "_INSERT_SIZE"))))
	res_ = predict(object = fit_, newdata = data_ %>% dplyr::select(c(paste0(target_names[i], "_READ_COUNT"), paste0(target_names[i], "_INSERT_SIZE"))))
	...
	observed[[i]] = data_ %>%
			...
			dplyr::select(c("Is_ctDNA", "Pr", paste0(target_names[i], "_READ_COUNT"), paste0(target_names[i], "_INSERT_SIZE"))) %>%
...
plot_ = do.call(rbind, observed) %>%
	...
	geom_point(stat = "identity", fill = NA, color = "#a50f15", alpha = .75, size = 3) +
...
pdf("../res/Number_Read_Pairs_by_Insert_Size_with_Posterior_Probability_Training_Set_HPV-16_Positive.pdf", width = 15/(1.35*1.1), height = 9/(1.25*1.1))
print(plot_)
dev.off()
...
pdf("../res/Number_Read_Pairs_by_Insert_Size_with_Posterior_Probability_Training_Set_HPV-16_Negative.pdf", width = 15/(1.35*1.1), height = 9/(1.25*1.1))
print(plot_)
dev.off()
...
plot_ = do.call(rbind, observed) %>%
	...
	geom_rect(data = do.call(rbind, simulated) %>%
		  ...
		  alpha = .25, inherit.aes = FALSE) +
	scale_fill_continuous(low = "#e7e1ef", high = "#c51b8a", breaks = c(.1, .3, .5, .7, .9)) +
	geom_point(stat = "identity", fill = NA, color = "#a50f15", alpha = .55, size = 2) +
	scale_shape_manual(values = c("+ve" = 3, "-ve" = 4)) +
	xlab("Number of Read Pairs Aligned") +
	ylab("Mean Insert size (bp)") +
	scale_x_continuous(breaks = log10(c(0, 1E1, 1E2, 1E3, 1E4, 1E5, 1E6)+1),
	...
	theme(axis.title.x = element_text(margin = margin(t = 20)),
	      axis.title.y = element_text(margin = margin(r = 20)),
	      panel.spacing = unit(1, "lines")) +
	guides(shape = guide_legend(title = "ctDNA", order = 1, override.aes = list(alpha = 1)),
	       fill = guide_colorbar(title = "Posterior\nProbability", order = 2))

pdf("../res/Number_Read_Pairs_by_Insert_Size_with_Posterior_Probability_Test_Set_HPV-16_Positive.pdf", width = 15/(1.35*1.1), height = 9/(1.25*1.1))
print(plot_)
dev.off()
...
pdf("../res/Number_Read_Pairs_by_Insert_Size_with_Posterior_Probability_Test_Set_HPV-16_Negative.pdf", width = 15/(1.35*1.1), height = 9/(1.25*1.1))
print(plot_)
dev.off()
